...
class ProjectController extends Controller
{
            ...
            ->addColumn('action', function($row){
                   ...
                   if ($row->status == '1') {
                    ...
                    if ($row->approval_status == '2') {
                        $btn .= '<a href="'.route('admin.project_approve',['project_id'=>encrypt($row->id),'status' => encrypt(1)]).'" class="btn btn-success btn-sm">Approve</a>';
                    }
                    return $btn;
            })
    ...
    public function projectApprove($project_id,$status)
    {
        try {
            $project_id = decrypt($project_id);
            $status = decrypt($status);
